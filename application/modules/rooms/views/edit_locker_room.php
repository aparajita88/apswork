...
<section role="main" class="content-body">
	...
	<div class="row">
			...
			<h2 class="panel-title">Edit Locker Room</h2>
			</div>
									<header class="panel-heading">
										...
										<?php 
										$row=$locker_room[0];
										$focus=explode(",",$locker_room[0]['locker_type']);
										//print_r($focus);
										// $attributes = array( 'method'=>'post','name' => 'form','id' => 'form','class'=>"orm-horizontal form-bordered");
          // echo  form_open_multipart('index.php/rooms/edit_locker_room/'.$row['locker_id'], $attributes);?>
		  <form action="<?php echo base_url();?>index.php/rooms/edit_locker_room/<?php echo $row['locker_id'];?>" method="post" enctype="multipart/form-data" name="form" id="form" class="orm-horizontal form-bordered" >
...
<a><i class="icon-camera"></i></a><br><input onchange="readURL(this,1);" type="file" name="ListeeTypeImage" id="ListeeTypeImage">
						 ...
						 <div class="form-group">
							...
							<div class="col-md-6 chk">
								 <?php 
								 $all_game_types = array("small", "medium", "large");
								
								 
								 foreach($all_game_types as $key=>$value) {
										//echo in_array($value['name'],$focus);
										
										if(in_array($value,$focus))
										{
											
											echo '<input class="sel" type="checkbox" name="chkbox[]" value="'.$value.'" checked="checked" />  '.$value.'<br/>';
										}
										else
										{
											echo '<input class="sel" type="checkbox" name="chkbox[]" value="'.$value.'" />  '.$value.'<br/>';
										}
