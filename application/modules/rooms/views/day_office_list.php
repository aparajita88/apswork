...
<section role="main" class="content-body">
	...
	<style>
		...
		<section class="panel">
			...
			<h2 class="panel-title">Day Offices</h2>
			</div>
			<br>
						...
						<div id="error" style="display:none"  ></div>
								...
								<div class="panel-actions">
									<a href="<?php echo $this->config->item('base_url');?>index.php/rooms/add_day_office" class="panel-action" title="Add Day Office"><i class="fa fa-plus"></i></a>
			...
			<div id="table_result_ajax" >
								<table class="table table-bordered table-striped mb-none" id="datatable-default">
									...
									<tbody>
											<?php //print_r($day_rooms);
											if(!empty($day_rooms)){
		  	foreach($day_rooms as $row)
			{
		     ?>
										<tr class="gradeX">
											...
											<td id="statusId<?php echo $row['dayoffice_id']; ?>"  class="center hidden-phone">
												<a title="<?php if($row['status']==1){ echo 'Active'; } else{ echo 'Inactive';} ?>"
											class="demo-basic" href="javascript:void(0)" onclick="changeroomStatus('<?php echo $row['dayoffice_id']; ?>','<?php echo $row['status']; ?>')">
												<?php if($row['status']==1){ echo 'Active'; } else{ echo 'Inactive';} ?>
												</a>
											</td>
											<td class="actions">
															<a href="<?php echo base_url();?>index.php/rooms/edit_day_office/<?php echo $row['dayoffice_id'];?>" ><i class="fa fa-pencil"></i></a>
														<!----	<a title="Add Image" style="margin-left: 4px;" class="on-default edit-row" href="<?php// echo base_url();?>index.php/vendor/add_classified_images/<?php //echo $row['classifiedId'];?>"><i class="fa fa-picture-o"></i></a>
															...
															<a href="<?php echo base_url();?>index.php/rooms/delete_day_office/<?php echo $row['dayoffice_id'];?>" onclick="return confirm('Are you sure you want to delete')"><i class="fa fa-trash-o"></i></a>
														</td>
										</tr>
										<?php }}else{?>
											
											<tr class="gradex"><td colspan="7">No meeting rooms Available.....</td></tr>
...
function changeroomStatus(id,status)
{
	//alert(id);
	//alert(js_site_url);
	var table='day_office';
	var pid='dayoffice_id';
	$.ajax({ 
      method: "POST", 
