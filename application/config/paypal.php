...
$db =& DB();
$db->where('settingsId','45240295-2d73-b1');
$db->select('payment_settings.*');
$query = $db->get('payment_settings');
$result = $query->result();
$paypal_row=$result[0];

if($paypal_row->Sandbox) $sandbx=TRUE; else $sandbx=FALSE;
/** 
 * Sandbox / Test Mode
 * -------------------------
 ...
 */
$config['APIUsername'] = $config['Sandbox']? 'paulpritam911_api1.gmail.com' : $paypal_row->APIUsername;
$config['APIPassword'] = $config['Sandbox'] ? '1396956962' : $paypal_row->APIPassword;
$config['APISignature'] = $config['Sandbox'] ? 'AiPC9BjkCyDFQXbSkoZcgqH3hpacA0DQr73PYNtYl48FHB2ACktL43ai' : $paypal_row->APISignature;
 ...
 */
$config['PayFlowUsername'] = $config['Sandbox'] ? 'SANDBOX_USERNAME_GOGES_HERE' : 'PRODUCTION_USERNAME_GOGES_HERE';
 ...
 */
$config['ApplicationID'] = $config['Sandbox'] ? 'APP-80W284485P519543T' : $paypal_row->ApplicationID;

/*
 * PayPal Developer Account Email Address
 * This is the email address that you use to sign in to http://developer.paypal.com
 */
$config['DeveloperEmailAccount'] = 'paulpritam911@gmail.com';
