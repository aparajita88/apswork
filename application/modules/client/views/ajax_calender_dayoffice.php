...
$( ".time_grid" ).click(function() {
 var time_grid = $( this );
  ...
  $.get(js_site_url+"index.php/client/getCurrentHour", function(info){
   // Get the hour of server time to check the slot without reload
	...
	var currentDate = '<?= date("d-m-Y"); ?>';
	  	if((parseInt(time.split(':')[0]) < parseInt(currentTime)) && (currentDate === date)){
	  		...
	  		$.ajax({
				type: "POST",
				url: js_site_url+"index.php/client/setBookDayOfficeRoom",
...
<script src="<?php echo $this->config->item('base_url');?>/application/modules/client/views/owl/owl.carousel-plugin.js"></script>
