...
<div class="min_booking_scroll item col-md-12">
	...
	<div class="col-md-8">
		...
		<?php  $attributes = array('name' => 'booking_online_atos','id' => 'booking_online_atos','class'=>"for_sign_up");
           echo form_open('index.php/indirect_client/setbookingDayOfficeRequestOnline/', $attributes);?>
			<input type="hidden" id='orderID' name="orderID" value=""/>
			<input type="hidden" id='price' name="price" value=""/>
			<input type="hidden" id='dayoffice_id' name="dayoffice_id" value=""/>
...
$(document).ready(function(){
	...
	if (dayoffice_id == ''|| dayoffice_id == null) {
		...
		$.ajax({
		type: "POST",
		url: js_site_url+"index.php/indirect_client/getCalenderDayOffice",
	...
	$("#bookRequest").click(function(){
	 ...
	 if (purpose == "") {
		...
		$.ajax({
			type: "POST",
			url: js_site_url+"index.php/indirect_client/setbookingDayOfficeRequest",
			data: { dayoffice_id: dayoffice_id, purpose: purpose, price:price},
			dataType: "json",
			success: function(data, textStatus, jqXHR) {
	...
	$("#bookOnline").click(function(){
		...
		if(purpose == "") {
			...
			return false;
	 		...
	 		$("#messageError").html('');
	 		$('#price').val(price);
	 		$('#dayoffice_id').val(dayoffice_id);
	 		$('#hidPurpose').val(purpose);
	 		if($('#price').val() != '' && $('#dayoffice_id').val() != '' && $('#hidPurpose').val() != ''){
...
function getdayofficeBybusiness(value){
	$.ajax({ 
	  method: "POST", 
	  url: js_site_url+"index.php/indirect_client/getdayofficeBybusiness/"+value, 
	  async: true, 
	  success: function(data) { 	  	
