             ...
             <div class="row">
                 <div class="col-md-12">
                    <div class="form-group sub_text" style="padding-left:15px;">
						...
						<?php  $attributes = array('name' => 'bookinglocker_online_atos','id' => 'bookinglocker_online_atos','class'=>"for_sign_up");
				           echo form_open('index.php/indirect_client/bookingLockerRequestOnline/', $attributes);?>
							<input type="hidden" id='orderID' name="orderID" value=""/>
							<input type="hidden" id='locker_type' name="locker_type" value=""/>
							<input type="hidden" id='start_date' name="start_date" value=""/>
							<input type="hidden" id='end_date' name="end_date" value=""/>
							<input type="hidden" id='price' name="price" value=""/>
							<input type="hidden" id='locker_id' name="locker_id" value=""/>
							<input type="hidden" id='locker_count' name="locker_count" value=""/>
                    </div> 
                 </div>
             </div>
       </div>
...
$(document).ready(function(){
    ...
    $.ajax({ 
      method: "POST", 
      url: js_site_url+"index.php/indirect_client/getLockerRoom/"+locker_id, 
      async: false, 
    ...
    $("#bookRequest").click(function(){
	 	$("#messageError").html('');
	 	if(CheckRadio() && chkStartDate() && chkEndDate() && chkLocNbr()){
			...
			$.ajax({
				type: "POST",
				url: js_site_url+"index.php/indirect_client/setbookingLockerRequest",
				data: { locker_room_id: locker_room_id, locker_type: locker_type, total_locker: total_locker, start_date: start_date, end_date: end_date, price: price},
				dataType: "json",
    ...
    $("#bookOnline").click(function(){
		if(CheckRadio() && chkStartDate() && chkEndDate() && chkLocNbr()){
		$("#messageError").html('');
		$('#locker_type').val($('input[name="size"]:checked').val());
		$('#start_date').val($("#datetimepicker6 input").val());
		$('#end_date').val($("#datetimepicker7 input").val());
		$('#price').val($('#totalIncTax').val());
		$('#locker_id').val($('#locker_room_id').val());
		$('#locker_count').val($("#loc_number").val());								
		$("#bookinglocker_online_atos").submit();
		}
	});
});
function CheckRadio(){
...
function getlockerBybusiness(value){
	$.ajax({ 
      method: "POST", 
      url: js_site_url+"index.php/indirect_client/getlockerBybusiness/"+value, 
      async: true, 
      success: function(data) { 
