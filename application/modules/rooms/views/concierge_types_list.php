...
<section role="main" class="content-body">
	...
	<section class="panel">
                        ...
                        <?php } ?>
							<div class="panel-body"><h2 class="panel-title">CONCIERGE TYPE LIST
							
							<a href="<?php echo $this->config->item('base_url');?>index.php/rooms/add_concierge_types" class="panel-action pull-right" title="Add Concierge Types"><i class="fa fa-plus"></i></a>
							...
							<div class="panel-body panel_body_top"><div id="table_result_ajax" >
								...
								<?php
								//print_r($business);
									if(!empty($concierge_types))
									{
									
									foreach($concierge_types as $row)
									{
								     ?>
										<tr class="gradeX">
											...
											<td id="statusId<?php echo $row['id']; ?>"  class="center hidden-phone">
												<a title="<?php if($row['status']==1){ echo 'Active'; } else{ echo 'Inactive';} ?>" class="demo-basic" href="javascript:void(0)" onclick="changeconciergetypeStatus('<?php echo $row['id']; ?>','<?php echo $row['status']; ?>')">
												<?php if($row['status']==1){ echo 'Active'; } else{ echo 'Inactive';} ?>
												</a>
											</td>
											<td class="actions">
															<a href="<?php echo base_url();?>index.php/rooms/edit_concierge_types/<?php echo $row['id'];?>" ><i class="fa fa-pencil"></i></a>													
															<a href="<?php echo base_url();?>index.php/rooms/delete_concierge_types/<?php echo $row['id'];?>" onclick="return confirm('Are you sure you want to delete')"><i class="fa fa-trash-o"></i></a>
											</td>
										</tr>
									<?php
									}
											...
											<tr class="gradex"><td colspan="6">No concierge types  Available.....</td></tr>
...
function changeconciergetypeStatus(id,status)
{
	//alert(id);
	//alert(js_site_url);
	var table='concierge_types';
	var pid='id';
	$.ajax({ 
      method: "POST", 
      url: js_site_url+"index.php/rooms/changeconciergetypeStatus", 
