...
<!-- start: page -->
<section class="panel">
<div class="panel-body"><h2 class="panel-title">Book Conference Room</h2></div>
<div class="panel-body panel_body_top">
	<div class="row booking_carousel_min_min">
			...
			<div class="form-group sub_text1">
			   <label for="inputDefault" class="col-md-12 control-label pad_left0">Business Centers :</label>
				<div class="col-md-12 pad_left0">
					<select  class="form-control mb-md" name="business" id="business" onchange="getconferenceBybusiness(this.value)" onclick="removeValidateHtml('business')" >
		...
		<div class="col-md-3 selection">
			<div class="form-group sub_text1">							
				<label for="inputDefault" class="col-md-12 control-label pad_left0">Conference Rooms :</label>
				<div class="col-md-12 pad_left0">
					<select  class="form-control mb-md" name="conference" id="conference" onchange="getconferenceByLocation('this.value')" onclick="removeValidateHtml('locker')" >
                <?php foreach($conference_data as $key=>$value) {
...
<div class="min_booking_scroll item col-md-12">
	...
	<div class="col-md-8">
		...
		<?php  $attributes = array('name' => 'booking_online_atos','id' => 'booking_online_atos','class'=>"for_sign_up");
           echo form_open('index.php/indirect_client/bookingConferrenceRequestOnline/', $attributes);?>
			<input type="hidden" id='orderID' name="orderID" value=""/>
			<input type="hidden" id='price' name="price" value=""/>
			<input type="hidden" id='conference_id' name="conference_id" value=""/>
			<input type="hidden" id='hidPurpose' name="hidPurpose" value=""/>
	</div>
	<div class="clearfix"></div>   
</div>
<div class="clearfix"></div>   
...
$(document).ready(function(){
	...
	if (conference_id == ''|| conference_id == null) {
		...
		$.ajax({
		type: "POST",
		url: js_site_url+"index.php/indirect_client/getCalender",
	...
	$("#bookRequest").click(function(){
	 ...
	 if (purpose == "") {
		...
		$.ajax({
			type: "POST",
			url: js_site_url+"index.php/indirect_client/setbookingRequest",
			data: { conference_id: conference_id, purpose: purpose, price:price},
			dataType: "json",
			success: function(data, textStatus, jqXHR) {
	...
	$("#bookOnline").click(function(){
		var conference_id 	= $("#conference").val();
	 	var purpose 		= $("#purpose").val();
		var price 			= $('#totalIncTax').val();
		if(purpose == "") {
			$("#messageError").html('<div style="margin-top:18px;" class="alert alert-danger fade in"><a title="close"  aria-label="close" data-dismiss="alert" class="close" href="#">×</a><strong>Please enter your purpose of booking</strong></div>');
			return false;
	 	}else if($("#totalIncTax").length == 0){
	 		$("#messageError").html('<div style="margin-top:18px;" class="alert alert-danger fade in"><a title="close"  aria-label="close" data-dismiss="alert" class="close" href="#">×</a><strong>Please select timeslot(s) !!</strong></div>');
			return false;
	 	}else{
	 		$("#messageError").html('');
	 		...
	 		$('#hidPurpose').val(purpose);
	 		if($('#price').val() != '' && $('#conference_id').val() != '' && $('#hidPurpose').val() != ''){
	 			$("#booking_online_atos").submit();
	 		} 
	 	}
	});
});
...
function getconferenceByLocation(){
	var conference_id = $("#conference").val();
	if (conference_id == '') {
		$("#messageError").html('<div style="margin-top:18px;" class="alert alert-danger fade in"><a title="close"  aria-label="close" data-dismiss="alert" class="close" href="#">×</a><strong>Sorry!</strong> no rooms available</div>');
	}else{
...
function conferenceroomvalidation(){		
	var conference = $("#conference").val();
	if(conference=='0'){
		alert('please choose any conference room.');
		$('#conference').addClass('redmessage');
		return false;
	}else{
		$('#conference').removeClass('redmessage');
		return true;
	}			
}      
function get_location(value){
	$.ajax({ 
	  method: "POST", 
	  url: js_site_url+"index.php/indirect_client/getlocationbycity/"+value,  
	  async: true, 
...
function get_business(value){
	...
	$.ajax({ 
	  method: "POST", 
	  url: js_site_url+"index.php/indirect_client/getBusinessByLocation/"+value+"/"+city, 
	  async: true, 
	  success: function(data) {   	
...
function getconferenceBybusiness(value){
	$.ajax({ 
	  method: "POST", 
	  url: js_site_url+"index.php/indirect_client/getconferenceBybusiness/"+value, 
	  async: true, 
	  success: function(data) { 	  	
		$("#conference").html(data.trim());      
	  } 
	});		
}          
</script>
