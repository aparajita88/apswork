...
<script type="text/javascript" src="<?php echo $this->config->item('base_url');?>assets/ckeditor/ckeditor.js"></script>
			<section role="main" class="content-body">
					<header class="page-header">
                        ...
                        <?php } ?>
							<div class="panel-body"><h2 class="panel-title">EDIT CITY :   <?php echo $city[0]['name']?></h2></div>					
							<div class="panel-body panel_body_top">
							<form  name="addAttiributesspeFrm" id="addAttiributesspeFrm" method="post"
								   action="<?php echo $this->config->item('base_url');?>index.php/location/edit_city_name/<?php echo $city[0]['cityId']?>">
							<fieldset>
                                <input type="hidden" name="cityId" value="<?= $city[0]['cityId'] ?>">
                                 <div class="form-group">
									...
									<select name="region_name" id="region_name" class="form-control">
                                        ...
                                        <?php foreach($region as $key=>$value) {
                                        	...
                                        	if($value['regionId'] == $city[0]['regionId'])
											{
												echo '<option value="'.$value['regionId'].'" selected>'.$value['name'].'</option>'; 
											}
											else{
												echo '<option value="'.$value['regionId'].'">'.$value['name'].'</option>';
											}
								...
								<div class="form-group">
									...
									<input type="text" name="attiributespeName" id="attiributespeName"
										   class="form-control" value="<?= $city[0]['name'] ?>">
									</div>
								</div>
