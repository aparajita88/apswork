			...
			for ($i = 0; $i < 7; $i++, $ts += 86400){
				...
				if($ts >= strtotime(date("d-m-Y"))){
					...
					for($j = 8; $j < 17; $j++){
						...
						if($meeting_data[0]['start_time'] <= $j && $j <= $meeting_data[0]['end_time']){
							$time = $j.':00 '.(($j < 12) ? 'AM' : 'PM');
							$html .= '<div class="time_grid '.$slotCss.'" data-flag= "1" data-price="'.$meeting_data[0]["price"].'" data-date="'.date("d-m-Y", $ts).'" data-time="'.(string)($j.':00').'" id="tme_"> '.$time.'</div>';
						}else {
							$time = $j.':00 '.(($j < 12) ? 'AM' : 'PM');
...
<div class="col-md-4">
		<div class="booking_picture">
				<div class="thumbnail-gallery">	
					<a class="img-thumbnail lightbox" id="hrefLocation" href="<?php echo base_url();?>assets/uploads/images/full/<?php echo $meeting_data[0]['imageName']; ?>" data-plugin-options='{ "type":"image" }'>
						<img class="img-responsive" src="<?php echo base_url();?>assets/uploads/images/medium/<?php echo $meeting_data[0]['imageName']; ?>">
						<span class="zoom">
				...
				<div class="scrollDiv">
					<div class="scrollDiv-content">
						<p><?php echo $meeting_data[0]['description'];?></p>
					</div>
				</div>
		</div> 
</div>
  ...
  $.get(js_site_url+"index.php/receptionist/getCurrentHour", function(info){
   // Get the hour of server time to check the slot without reload
	   ...
	   if((parseInt(time.split(':')[0]) < parseInt(currentTime)) && (currentDate === date)){
	  		...
	  		$.ajax({
				type: "POST",
				url: js_site_url+"index.php/receptionist/setBookMeetingRoom",
