			...
			<h2 class="panel-title"> Game Rooms</h2>
								...
								<select class="design_select" name="city" id="city"  onchange="get_location(this.value)" >
										...
										<!---<h2 class="panel-title">OFFICE SPACES</h2>-->
										 <select  class="design_select" name="location" id="location" onchange="get_business(this.value)" onchange="change_room_bylocation(this.value)">
								...
								<div class="panel-actions">
									<a href="<?php echo $this->config->item('base_url');?>index.php/rooms/add_game" class="panel-action" title="Add Feature"><i class="fa fa-plus"></i></a>
		  	...
		  	foreach($game_rooms as $row)
			{
		     ?>
										<tr class="gradeX">
											...
											<td id="statusId<?php echo $row['game_id']; ?>"  class="center hidden-phone">
												<a title="<?php if($row['status']==1){ echo 'Active'; } else{ echo 'Inactive';} ?>" class="demo-basic" href="javascript:void(0)" onclick="changeroomStatus('<?php echo $row['game_id']; ?>','<?php echo $row['status']; ?>')">
												<?php if($row['status']==1){ echo 'Active'; } else{ echo 'Inactive';} ?>
												</a>
											</td>
											<td class="actions">
															<a href="<?php echo base_url();?>index.php/rooms/edit_game_room/<?php echo $row['game_id'];?>" ><i class="fa fa-pencil"></i></a>
														<!--	<a title="Add Image" style="margin-left: 4px;" class="on-default edit-row" href="<?php// echo base_url();?>index.php/vendor/add_classified_images/<?php //echo $row['classifiedId'];?>"><i class="fa fa-picture-o"></i></a>
															...
															<a href="<?php echo base_url();?>index.php/rooms/delete_game_room/<?php echo $row['game_id'];?>" onclick="return confirm('Are you sure you want to delete')"><i class="fa fa-trash-o"></i></a>
											</td>
										</tr>
										<?php }}else{?>											
											<tr class="gradex"><td colspan="7">No rooms Available.....</td></tr>																													
											<?php }?>
