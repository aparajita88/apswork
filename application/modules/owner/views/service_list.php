...
<style>
		...
		<section class="panel">
			...
			<h2 class="panel-title"> Services</h2>
								...
								<select class="design_select" name="city" id="city"  onchange="get_location(this.value)" >
										...
										<!---<h2 class="panel-title">OFFICE SPACES</h2>-->
										 <select  class="design_select" name="location" id="location">
                                     ...
                                     <select  class="design_select" name="location" id="request">
                                        ...
                                        <option value="conceirge">conceirge request</option>
                                         <option value="courier">courier request</option>
                                        <option value="food">food request</option>
                                        <option value="staff">staff request</option>
			...
			<div id="table_result_ajax" >
                           ...
                           -->
                   Total service:   <label id="result"> <?php   echo $total_service; ?></label>  	
				        </table>
							</div>	
...
function get_result(){
	...
	//alert(location_id);
	//alert(city_id);
	if(location_id!='' || city_id!='' || request!='' ){
		//alert('1')
$.ajax({ 
      method: "POST", 
      url: "<?php echo base_url();?>index.php/owner/get_result_services", 
      data: {location_id:location_id,city_id:city_id,request:request }, 
      async: false, 
      success: function(data) { 
	 // alert(data);	
