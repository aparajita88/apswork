...
<script type="text/javascript" src="<?php echo $this->config->item('base_url');?>assets/ckeditor/ckeditor.js"></script>
		...
		<section role="main" class="content-body">
					...
					<section class="panel">
                        ...
                        <?php } ?>
							<div class="panel-body"><h2 class="panel-title">ADD MOVIE</h2></div>					
							<div class="panel-body panel_body_top">
							<form  name="addAttiributesspeFrm" id="addAttiributesspeFrm" method="post"
								   action="<?php echo $this->config->item('base_url');?>index.php/location/add_movie">
							<fieldset>
								<div class="form-group">
									...
									<?php if($hall){
										foreach($hall as $row){?>
                                      <option value="<?php echo $row['hallId'];?>"><?php echo $row['name'];?></option>
										<?php }}?>
									</select>
									</div>
								</div>
...
<script>
	$(document).ready(function(e){
		$("#submit").click(function(e){
			...
			var movie_hall = $("#movie_hall").val();
			if(movie_hall==""){
                $("#movie_hall").css("border-color","red");
				$("#hall_location").focus();
				e.preventDefault();

			}
			else if (_moviename == "") {
				$("#movieName").attr('placeholder','Please enter the movie name');
