...
class booking_info extends MY_Model {
	...
	public function get_booking_seat_info_bydate($start_date,$end_date){
		$sql="select * from book_floor_plan left join user where book_floor_plan.book_for_client=user.userId and IsApproved='1' and ((start_date between '".date('Y-m-d',strtotime($start_date))."' and '".date('Y-m-d',strtotime($end_date))."') or (end_date between '".date('Y-m-d',strtotime($start_date))."' and '".date('Y-m-d',strtotime($end_date))."'))";
		$query=$this->db->query($sql); 
		return $query->result_array();
	}
 ...
 return $query->result_array();	
	...
	public function get_booking_info_forgameroom($tbname,$legal_data){
		$this->db->select('book_play_room.*,game_room.name as room_name,cities.name as cites,locations.name as location,company.company_name,user.FirstName,user.LastName');
		$this->db->from('book_play_room');
		$this->db->join('game_room','book_play_room.play_room_id=game_room.game_id');
		$this->db->join('user','user.userId=game_room.addedBy');
		$this->db->join('company','company.id=book_play_room.company_id');
